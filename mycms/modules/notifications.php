	...
	class notifications extends krn_abstract{
		...
		function GetNotificationsHtml(){		
			...
			$result=strtr($result,array(
				'<%LASTNOTIFICATION%>'	=> $last_notification?$last_notification['Text']:'No new notifications',
				//'<%MORE%>'				=> count($notifications_arr)>1?' &nbsp; <span class="dashed" onclick="Popup(\'notifications\',850);">'.(count($notifications_arr)-1).' more</span>':''
				'<%MORE%>'				=> ($_GET['module']=='notifications' || count($notifications_arr)<1)?'':' &nbsp; <a href="index.php?module=notifications">show all</a>'
			));
			return $result;
		}
		...
		function Browse(){
			$table=new BrowserTable(array('Notification','Date','Read'));
			...
			foreach($notifications_arr as $notification){
				$row=array(
					...
					$notification['Viewed']?'yes':'<strong style="font-weight: 600;">no</strong>'
				);
			...
			$result=strtr($result,array(
				'<%DOCUMENT_TITLE%>'		=> 'Notofications',
				'<%BREAD_CRUMBS%>'			=> '<a href="index.php?module=notifications">Notofications</a> &rarr; ',
				'<%TOOLS_RIGHT_TOP%>'		=> $toolRop,
				'<%FILTER%>'				=> '',
				'<%ACTION%>'				=> 'index.php#',
				'<%TABLE%>'					=> $table->GetTable(),
				'<%TOOLS_LEFT_BOTTOM%>'		=> GetButton('Delete',false,'ntfDeleteConfirm(this);'),
				'<%TOOLS_RIGHT_BOTTOM%>'	=> ''
			));
			return $result.$navMn;
		}

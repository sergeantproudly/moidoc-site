...
class records extends krn_abstract{
	...
	function BrowseDocuments(){
		...
		$table->AddSimpleHeaderRow(array('Title'));
		...
		$result=strtr($result,array(
			'<%DISPLAYTOP%>'			=> '',
			'<%DOCUMENT_TITLE%>'		=> 'Site management',
			'<%BREAD_CRUMBS%>'			=> '',
			'<%TOOLS_RIGHT_TOP%>'		=> '',
			'<%FILTER%>'				=> '',
			'<%ACTION%>'				=> 'index.php#',
	...
	function Browse($document=array(),$params=array()){
		...
		if($params['parent_record_field'] && ($params['parent_record_id'] || $params['subst_record_id'])){
			...
			$query = 'SELECT '.$document['IdElement'].', `'.implode('`, `', array_diff(array_keys($fields), array(''))).'` '
		...
		if($params['parent_record_field'] && $params['parent_record_id']){
			...
			$toolEdit=GetButton('Edit',false,'recEdit(this,'.$document['Id'].', {parent_record_id: '.$params['parent_record_id'].','.($params['subst_record_id']?'subst_record_id: '.$params['subst_record_id'].',':'').' parent_record_field: \''.$params['parent_record_field'].'\'' . (count($parents) ?  ', ' . implode(', ', $parents) : '') . '});');
			
		}else{
			$toolAdd=strtr(LoadTemplate('tool_add'),array(
				'<%ACTION%>'	=> 'recAdd(this,'.$document['Id'].');'
			));
			$toolEdit=GetButton('Edit',false,'recEdit(this,'.$document['Id'].');');
		}
		...
		*/
		...
		$breadCrumbs='<a href="index.php?module=records">Site management</a> &rarr; ';
		...
		$result=strtr($result,array(
			...
			'<%ACTION%>'				=> 'index.php#',
			'<%TABLE%>'					=> $table->GetTable(),
			'<%TOOLS_LEFT_BOTTOM%>'		=> ($document['CanEdit']?$toolEdit:'') .' '.($document['CanDelete']?GetButton('Delete',false,'recDeleteConfirm(this,'.$document['Id'].');'):''),
	...
	function View(){
		...
		foreach($fields as $name=>$info){
			...
			switch($info['Type']){
				...
				break;
				case 9:
					$query = 'SELECT ' . $elementProperties[91] .' FROM ' . $elementProperties[96] . ' tr LEFT JOIN ' . $elementProperties[90] . ' ts ON tr.' . $elementProperties[98] . ' = ts.' . $elementProperties[92] . ' WHERE ' . $elementProperties[97] . ' = ' . $this->record['Id'];
					$res = dbDoQuery($query);
					$items = [];
					while($item = dbGetRecord($res)) {
						$items[] = $item[$elementProperties[91]];
					}
					$data = implode($items, ', ');
				break;
				default:
	...
	function Add(){
		...
		$result=strtr($result,array(
			'<%DOCUMENT_TITLE%>'	=> 'Add record'.($count>1?'s':''),
			'<%BREAD_CRUMBS%>'		=> '<a href="index.php?module=records">Site management</a> &rarr; <a href="index.php?module=records&document_id='.$this->document['Id'].'">'.$this->document['Title'].'</a> &rarr; ',
			'<%CONTENT%>'			=> $content,
			...
			foreach($ids as $id){
				...
				foreach($fields as $name=>$info){
					...
					if($elementProperties[2]&&dbGetValueFromDb('SELECT COUNT(Id) FROM `'.$this->document['Table'].'` WHERE `'.$name.'`="'.$_POST[$name][$id].'"',__FILE__,__LINE__)){
						$errorInner=true;
						$javascript.='stxAddErrorStatus(gei(\''.$name.$id.'\'),\'There is already a record with this value\');';
					}
					if(!$errorInner){
						switch($info['Type']){
							case 1:
		...
		// single
			...
			foreach($fields as $name=>$info){
				...
				if($elementProperties[2]&&dbGetValueFromDb('SELECT COUNT(Id) FROM `'.$this->document['Table'].'` WHERE `'.$name.'`="'.$_POST[$name].'"',__FILE__,__LINE__)){
					$error=true;
					$javascript.='stxAddErrorStatus(domP(form.'.$name.'),\'There is already a record with this value\');';
				}
				if(!$error){
					switch($info['Type']){
						case 1:
	...
	function Edit(){
			...
			foreach($fields as $name=>$info){
				...
				switch($info['Type']){
					...
					case 9:
						...
						$defaultTitle='Selected '.$counter.' '.Word125($counter,'value','values','values');
		...
		$result=LoadTemplate('base_editor');
		$result=strtr($result,array(
			'<%DOCUMENT_TITLE%>'	=> 'Edit record'.(count($ids)>1?'s':''),
			'<%BREAD_CRUMBS%>'		=> '<a href="index.php?module=records">Site management</a> &rarr; <a href="index.php?module=records&document_id='.$this->document['Id'].'">'.$this->document['Title'].'</a> &rarr; ',
			'<%CONTENT%>'			=> $content,

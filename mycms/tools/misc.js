...
function docEdit(sender){
	...
	if(fstCb){
		...
		Popup('message',300,{'header':'Hmm,','message':'Nothing seems to be selected'});
	}
}
...
function docDeleteConfirm(sender,id){
	if(hasClass(sender,'ico')){
		var hint=Hint(sender,'<div class="confirm">Are you sure?<br/><span class="dashed">yes</span> <span class="dashed">no</span></div>',150);
...
function recDeleteConfirm(sender,document_id,id){
	if(hasClass(sender,'ico')){
		...
		for(var i=0;i<form.elements.length;i++){
			...
			if(inp.type=='checkbox' && inp.name!='check_all'){
				if(inp.checked){
					Popup('delete_confirm',350,{'thing':'records','action':'recDeleteSendForm('+document_id+');'});
					return true;
				}
			}
		}
...
function ntfDeleteConfirm(sender,id){
	if(hasClass(sender,'ico')){
		...
		for(var i=0;i<form.elements.length;i++){
			...
			if(inp.type=='checkbox' && inp.name!='check_all'){
				if(inp.checked){
					Popup('delete_confirm',350,{'thing':'notifications','action':'ntfDeleteSendForm();'});
					return true;
				}
			}
		}

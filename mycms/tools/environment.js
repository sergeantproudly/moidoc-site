...
function envMultiSelectItemSet(sender){
	...
	$(inp).val('Selected '+vals.length+' '+word125(vals.length,'value','values','values'));
}
...
function envInitWysiwyg(wysiwyg){
	...
	var config={
		toolbar : [
					['Maximize', 'Source', '-', 'Bold', 'Italic'],
					['Link', 'Unlink','-', 'Image','-','NumberedList','BulletedList','Table'],
					['Styles', 'Format', 'TextColor','RemoveFormat'],
	    ...
	    language : 'en',
	    stylesSet : [
	    	{'name' : 'Bold', element: 'p', attributes: {'class': 'bold'}}
	    ]
	};

...
function stxCheckNotEmpty(fld){
	if(fld.value)return true;
		...
		if(mode=1){
			stxAddErrorStatus(elem,'This field cannot be empty');
		}
		return false;
	}
}
function stxCheckTheSame(fld,mode){
	...
	if(fld.value==fld2.value)return true;
		...
		addClass(elem,'inp-error');
		if(mode=1){
			stxAddErrorStatus(elem,'The password does not match the repeated one');
		}
		return false;
	}
}
function stxCheckUploaded(fld,mode){
	...
	if(!fld.getAttribute('uploading'))return true;
		...
		addClass(elem,'inp-error');
		if(mode=1){
			stxAddErrorStatus(elem,'Wait for the file to download');
		}
		return false;
	}
}
